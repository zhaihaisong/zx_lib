...
//      }
    ...
    public Rect getFramingRect() {
        try {
            ...
            int windowWidth = screenResolution.x/5*3;


            int leftOffset = (screenResolution.x - windowWidth) / 2;

            int topOffset;
//            if (FRAME_MARGINTOP != -1) {
//                topOffset = FRAME_MARGINTOP;
//            } else {
                topOffset = (screenResolution.y - windowWidth) / 4;
                topOffset = windowWidth / 2;
//            }
            framingRect = new Rect(leftOffset, topOffset, leftOffset + windowWidth, topOffset + windowWidth);
            // }
            return framingRect;
        } catch (Exception e) {
            e.printStackTrace();
